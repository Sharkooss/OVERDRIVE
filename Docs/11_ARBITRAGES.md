...
| 2026-08-21 | **D59** | Durée de hit-stop « en temps réel » obtenue par un timer monde de durée `RealDuration × Dilation` | Rédigée au J10bis — cf. `04_ROADMAP.md` |
| 2026-08-21 | **D60** | **`Knockback_MinImpulse` garde le décollage, `WallSlam_MinImpactSpeed` garde les dégâts muraux** | Les deux clés existaient et testaient la même chose (`SPEC_COMBAT §7.2` vs `07_TUNING §13`) : deux vérités concurrentes, ce que **R3** interdit. Détail dans `SPEC_COMBAT §7.2` |
| 2026-08-21 | **D61** | **Le knockback décroît en exponentielle** (`Knockback_AirDrag` → `CMC.FallingLateralFriction`) au lieu d'une vitesse constante, et `Melee_Knockback` passe 3500 → 2800 | Retour de Louis manche en main : *« il va beaucoup trop loin »*, *« je veux un vrai impact… il prend très vite de la vitesse mais l'ennemi est grand et lourd donc il freine très vite »*. Une vélocité constante = profil « poids plume » |
| 2026-08-21 | **D62** | **Les 6 clés de knockback/wall slam déménagent de `BPC_KnockbackReceiver` vers `PDA_EnemyData`** ; les variables du composant deviennent des caches non `Instance Editable` | Un acteur déjà posé fige sa copie des défauts d'un composant : mesuré, les 6 Grunts ignoraient le nouveau réglage (`12_PIEGES §5.75`). C'est aussi ce qu'exigeait `10_DEFINITION_OF_DONE §5`. Effet de bord bienvenu : la traînée devient **per-ennemi** |
| 2026-08-21 | **D63** | **La direction du knockback est le YAW seul** (`MakeRotator(0, ControlRotation.Yaw, 0)`), plus la caméra complète ; `Melee_Knockback` 2800 → **4500** | Louis, 2ᵉ playtest : *« quand je l'envoie à la verticale, genre je regarde le ciel, il part méga super haut »*. Nez au zénith, `Impulse.Z = Knockback + KnockbackUp` = 3100 uu/s → apex ~49 m, et `Knockback_AirDrag` ne freine que le latéral. Viser en l'air ne sert à rien : `SPEC_COMBAT §7.3` exclut déjà sols et plafonds du slam. Bénéfice : toute l'impulsion part dans la **distance**, ce qui règle aussi *« pas assez de recul »* |
| 2026-08-21 | **D64** | **Le pitch de visée revient — mais sur l'axe Z SEULEMENT** : `Lift = Melee_KnockbackPitchBoost × sin(Clamp(Pitch, 0°, 90°))`, ajouté à `Melee_KnockbackUp`. `Melee_Knockback` 4500 → **5500** | Louis, 3ᵉ playtest : *« encore un poil plus de force »* et *« un peu plus de verticalité, il rase un peu trop le sol quand on vise haut »*. `D63` avait supprimé toute influence du regard vertical, c'était trop absolu. Les deux axes restent **indépendants** : viser haut **ajoute** du Z sans **retirer** d'horizontale, et le vertical est borné par une clé — donc `D63` ne peut pas revenir |
| 2026-08-21 | **D65** | **La traînée se règle CONTRE le temps de vol, pas seule** : `Knockback_AirDrag` 2.5 → **0.6**, `Melee_KnockbackUp` 300 → **600**, `Melee_Knockback` 5500 → **2700**, `Knockback_MaxFlightTime` 1.2 → **2.5**, `WallSlam_MinImpactSpeed` 900 → **700** | Louis, 4ᵉ playtest, croquis à l'appui : la trajectoire n'était pas une parabole mais « fort à l'horizontale, puis ralentit et monte, puis redescend d'un coup ». Cause : constante de temps horizontale `1/k = 0.4 s` contre un vol de 1.6 s — l'horizontale mourait au premier quart, l'ennemi finissait de monter sur place puis tombait à la verticale. **Règle posée : `1/AirDrag ≥ t_air`** |
| 2026-08-21 | **D66** | **Le hitmarker fond en entrée et en sortie, et la jauge de chaleur se remplit en continu** — les deux pilotés par le `Tick` de leur widget, pas par une `WidgetAnimation` | Demande de Louis (5ᵉ playtest). Aucun toolset MCP ne crée de `WidgetAnimation` (même famille que `SoundCue` au J11 et `AnimBlueprint`) : le `Tick` du widget rend exactement la même chose, se règle par une clé de tuning, et se remplacera par une animation timeline sans toucher aux appelants. `ShowHitmarker` et `SetHeat` gardent leur signature |
